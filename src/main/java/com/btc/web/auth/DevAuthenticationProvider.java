...
public class DevAuthenticationProvider implements AuthenticationProvider {
    ...
    private List<String> devAuthorities;

    @Value("${sec.dev.user}")
    private String user;

    @Value("${sec.dev.password}")
    private String password;

    @Value("${sec.dev.authorities}")
    private String authorities;

    @PostConstruct
    public void init() {
        md5PasswordEncoder = new Md5PasswordEncoder();
        devAuthorities = Lists.newArrayList(Splitter.on(",").omitEmptyStrings().trimResults().split(authorities));
    }
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (user.equals(passedInUsername) && password.equals(encodedPassword)) {
            ...
            Set<GrantedAuthority> grantedAuthorities = devAuthorities.stream().map(SimpleGrantedAuthority::new)
